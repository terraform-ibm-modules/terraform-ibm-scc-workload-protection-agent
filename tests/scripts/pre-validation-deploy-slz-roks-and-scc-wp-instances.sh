...
(
  ...
  cluster_resource_group_name_var_name="cluster_resource_group_name"
  cluster_resource_group_name_value=$(terraform output -state=terraform.tfstate -raw cluster_resource_group_name)
  ...
  jq -r --arg region_var_name "${region_var_name}" \
        ...
        --arg cluster_resource_group_name_var_name "${cluster_resource_group_name_var_name}" \
        --arg cluster_resource_group_name_value "${cluster_resource_group_name_value}" \
        --arg access_key_var_name "${access_key_var_name}" \
        --arg access_key_value "${access_key_value}" \
        '. + {($region_var_name): $region_var_value, ($cluster_id_var_name): $cluster_id_value, ($cluster_resource_group_name_var_name): $cluster_resource_group_name_value, ($access_key_var_name): $access_key_value}' "${JSON_FILE}" > tmpfile && mv tmpfile "${JSON_FILE}" || exit 1
